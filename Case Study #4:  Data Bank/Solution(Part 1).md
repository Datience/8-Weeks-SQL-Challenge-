...
SELECT * FROM (VALUES 
  ...
  ) as  customer_transactions (customer_id, txn_date, txn_type, txn_amount);
```
...
SELECT customer_id
, node_id
, sum(date_diff) AS sum_date_diff from
(SELECT 
...
, region_name
,PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY sum_date_diff)   
                            OVER (PARTITION BY region_id) as median
,PERCENTILE_CONT(0.8) WITHIN GROUP (ORDER BY sum_date_diff)   
                            OVER (PARTITION BY region_id) as eightiethpercentile
,PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY sum_date_diff)   
                            OVER (PARTITION BY region_id) as nintyfifthpercentile
FROM (
SELECT customer_id
, node_id
, region_id
...
WHERE end_date != '9999-12-31'
GROUP BY customer_id, node_id, start_date, end_date, c.region_id, region_name
) AS T 
